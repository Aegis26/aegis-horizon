...
import { and, eq, lte, ne, notExists } from "drizzle-orm";
import {
  db,
...
export async function publishWebhookEvent(orgId: string, eventType: "lead.created" | "lead.updated" | "opportunity.created" | "opportunity.updated", entityId: string, data: Record<string, unknown>) {
  return withOrganizationSharedLock(orgId, () =>
    publishWebhookEventUnlocked(orgId, eventType, entityId, data),
  );
}
async function publishWebhookEventUnlocked(orgId: string, eventType: "lead.created" | "lead.updated" | "opportunity.created" | "opportunity.updated", entityId: string, data: Record<string, unknown>) {
  if (await isOrganizationDeletionActive(orgId)) return;
...
async function deliver(id: string) {
  ...
  return withOrganizationSharedLock(delivery.orgId, () => deliverUnlocked(id));
}
async function deliverUnlocked(id: string) {
  const [delivery] = await db.select().from(webhookDeliveries).where(eq(webhookDeliveries.id, id)); if (!delivery || delivery.status === "delivered") return;
  if (await isOrganizationDeletionActive(delivery.orgId)) return;
...
export function startWebhookScheduler() { if (scheduler) return; scheduler = setInterval(() => void (async () => { const rows = await db.select({ id: webhookDeliveries.id }).from(webhookDeliveries).where(and(
  ...
  notExists(
    db.select({ id: organizationDeletionLedger.id })
      ...
      .where(and(
        eq(organizationDeletionLedger.organizationId, webhookDeliveries.orgId),
        ne(organizationDeletionLedger.status, "completed"),
      )),
  ),
)).limit(25); await Promise.all(rows.map((row) => deliver(row.id))); })().catch((err) => logger.error({ err }, "Webhook worker failed")), 60_000); scheduler.unref(); }
export async function testWebhook(hook: typeof webhooks.$inferSelect) {
  return withOrganizationSharedLock(hook.orgId, async () => {
    if (await isOrganizationDeletionActive(hook.orgId)) {
    ...
    const [delivery] = await db.insert(webhookDeliveries).values({
      ...
      payload: { type: "webhook.test", data: { message: "Aegis Horizon webhook test" } },
    }).returning();
    await deliverUnlocked(delivery.id);
